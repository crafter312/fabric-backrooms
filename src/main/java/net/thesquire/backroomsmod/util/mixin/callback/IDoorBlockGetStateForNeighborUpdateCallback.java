...
public interface IDoorBlockGetStateForNeighborUpdateCallback {

    Event<IDoorBlockGetStateForNeighborUpdateCallback> EVENT = EventFactory.createArrayBacked(IDoorBlockGetStateForNeighborUpdateCallback.class,
            (listeners) -> (state, direction, neighborState, world, pos, neighborPos, ci) -> {
                for (IDoorBlockGetStateForNeighborUpdateCallback listener : listeners) {
