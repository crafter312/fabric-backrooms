...
public class ModFlatOreFeature extends Feature<OreFeatureConfig> {

    public ModFlatOreFeature(Codec<OreFeatureConfig> codec) {
        super(codec);
    }
    ...
    public boolean generate(FeatureContext<OreFeatureConfig> context) {
        ...
        for (int s = n; s <= n + q; ++s) {
            for (int t = p; t <= p + q; ++t) {
                ...
                return this.generateVeinPart(structureWorldAccess, random, oreFeatureConfig, d, e, h, j, o, o, n, o, p, q, r);
            }
        }
        return false;
    }
    ...
    protected boolean generateVeinPart(StructureWorldAccess world, Random random, OreFeatureConfig config, double startX, double endX, double startZ, double endZ, double startY, double endY, int x, int y, int z, int horizontalSize, int verticalSize) {
        ...
        for (k = 0; k < j - 1; ++k) {
            ...
            for (int m = k + 1; m < j; ++m) {
                if (ds[m * 4 + 3] <= 0.0 || !((h = ds[k * 4 + 3] - ds[m * 4 + 3]) * h >
                                        (d = ds[k * 4] - ds[m * 4]) * d +
                                        (e = ds[k * 4 + 1] - ds[m * 4 + 1]) * e +
                                        (g = ds[k * 4 + 2] - ds[m * 4 + 2]) * g)) continue;
                if (h > 0.0) {
                    ds[m * 4 + 3] = -1.0;
                    continue;
                }
                    ...
                    for (int v = o; v <= r; ++v) {
                        ...
                        block11: for (int aa = p; aa <= s; ++aa) {
                            ...
                            for (OreFeatureConfig.Target target : config.targets) {
                                if (!ModFlatOreFeature.shouldPlace(blockState, chunkSectionCache::getBlockState, random, config, target, mutable)) continue;
    ...
    public static boolean shouldPlace(BlockState state, Function<BlockPos, BlockState> posToState, Random random, OreFeatureConfig config, OreFeatureConfig.Target target, BlockPos.Mutable pos) {
        ...
        return OreFeature.isExposedToAir(posToState, pos) && ModFlatOreFeature.shouldNotDiscard(random, config.discardOnAirChance);
    }
